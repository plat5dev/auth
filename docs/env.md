...
2. Invite link is this issuer's `/authorize` with `invite_token` (and a `redirect_uri` that is `INVITE_REDEEM_URL` / the console callback, allowlisted).
3. Auth stores the token in an HttpOnly cookie so it survives password login **and** register.
4. After success, Auth appends `invite_token` to the OAuth `Location`. If `IDENTITY_INVITE_REDEEM_URL` is set, Auth also POSTs `{ token, user_id }` to that public URL.
